...
public class CustomerDatabase
{
	...
	public Customer getAccount(String checkPassword){
		...
		for (int i = 0; i < customerArray.length; i++){
			if (customerArray[i].getPassword() == checkPassword){
				returnID = i;
            }
        }
	...
	private String CompileAsString(){
		...
		for (int i = 0; i < customerArray.length; i++){
			toReturn += toReturn + customerArray[i].getName() + "," + customerArray[i].getPassword() + "," + customerArray[i].accountType + ",";
			if (customerArray[i].accountType == "Manager"){
				toReturn += toReturn + "NULL;";
            } else {
				toReturn += toReturn + customerArray[i].getAmountspent() + ";";
            }
        }
