...
public class Inventory {
   ...
   */
    public Inventory() {
        // read inventory data from file (if any)
        try {
            InputStream is = getClass().getResourceAsStream("inventory.txt");
            BufferedReader reader = new BufferedReader(new InputStreamReader(is));
            ...
            String[][] result = inventoryData(data);
            products = result;
            reader.close();
        } catch (IOException e) {
            System.out.println("Failed to read inventory data from file.");
        }
    }
    ...
    public void updateItem(String name, double price, int quantity) {
        // find the product with the given name and update its quantity
        for (int i = 0; i < numProducts; i++) {
            if (products[i][0].equals(name)) {
    ...
    private void groupProductsToFile() {
        try {
            // open file for writing
            PrintWriter writer = new PrintWriter(new FileWriter("inventory_grouped.txt"));
